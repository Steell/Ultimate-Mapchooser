...
//Handles the Change Map option in the menu.
public UMCMenu_NextMap(Handle:topmenu, TopMenuAction:action, TopMenuObject:objectID, client, String:buffer[], maxlength)
{
	if (action == TopMenuAction_DisplayOption)
	{
...
DoAutoMapChange(client, UMC_ChangeMapTime:when)
{
	...
	if (UMC_GetRandomMap(map_kv, umc_mapcycle, INVALID_GROUP, nextMap,
...
Handle:CreateMapMenu(MenuHandler:handler, const String:group[], bool:limits, client)
{
	...
	decl String:display[MAP_LENGTH+4];
