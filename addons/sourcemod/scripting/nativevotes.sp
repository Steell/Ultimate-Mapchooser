...
{
	NativeVotesOverride VoteType;
	bool VoteEnabled;
}

PrivateForward g_CallVotes[NativeVotesOverride_Count][CallVoteForwards];
		...
		case 0:
		{
			...
			for (int i = 1; i < sizeof(g_CallVotes); i++)
			{
				if (GetForwardFunctionCount(g_CallVotes[i][CallVote_Forward]) > 0)
				{
					...
					int pos = FindVoteInArray(hVoteTypes, view_as<NativeVotesOverride>(i), voteData);

					if (pos > -1)
					{
						voteData.VoteEnabled = true;

#if defined LOG
						LogMessage("Forcing vote type to visible: %d", i);
#endif

						hVoteTypes.SetArray(pos, voteData);
					}
					else
					{
#if defined LOG
						...
						voteData.VoteType = view_as<NativeVotesOverride>(i);
						voteData.VoteEnabled = true;
						hVoteTypes.PushArray(voteData);
					}
				}
			}
...
stock int FindVoteInArray(ArrayList myArray, NativeVotesOverride value, CallVoteListData voteData)
{
	for (int i = 0; i < myArray.Length; i++)
	{
		myArray.GetArray(i, voteData);

		if (voteData.VoteType == value)
		{
			return i;
		}
	}
...
{
	CallVoteListData voteData;
	int pos = FindVoteInArray(myArray, value, voteData);
	return (pos > -1 ? voteData.VoteEnabled : false);
}
...
void PerformVisChecks(int client, ArrayList hVoteTypes)
{
	...
	{
		CallVoteListData voteData;
		hVoteTypes.GetArray(i, voteData);
		...
		Call_StartForward(g_CallVotes[voteData.VoteType][CallVote_Vis]);
		Call_PushCell(client);
		Call_PushCell(voteData.VoteType);
		Call_Finish(hide);
		...
		if (hide >= Plugin_Handled)
		{
			...
			{
				voteData.VoteEnabled = false;
				hVoteTypes.SetArray(i, voteData);
			}
		}
	}
}
...
// native bool:NativeVotes_GetItem(Handle:vote, position, String:infoBuf[], infoBufLen, String:dispBuf[]="", dispBufLen=0);
...
// native NativeVotes_DisplayRawPassToOne(client, NativeVotesPassType:passType,
//     const String:details[]="", team=NATIVEVOTES_ALL_TEAMS);
...
// native NativeVotes_RegisterVoteCommand(NativeVotesOverride:overrideType, NativeVotes_CallVoteHandler:callHandler,
//     NativeVotes_CallVoteVisCheck:visHandler=INVALID_FUNCTION);
...
// native NativeVotes_UnregisterVoteCommand(NativeVotesOverride:overrideType, NativeVotes_CallVoteHandler:callHandler,
//     NativeVotes_CallVoteVisCheck:visHandler=INVALID_FUNCTION);
