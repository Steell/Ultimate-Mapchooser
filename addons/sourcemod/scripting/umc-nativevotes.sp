...
Handle:BuildVoteMenu(Handle:vote_items, NativeVotes_VoteHandler:callback, NativeVotesType:type, const String:title[]="")
{
	...
	//Begin creating menu
	new Handle:menu = NativeVotes_Create(Handle_VoteMenu, type, NATIVEVOTES_ACTIONS_DEFAULT
		| MenuAction_VoteCancel | MenuAction_Display | MenuAction_DisplayItem);
...
public Handle_MapVoteResults(Handle:menu, num_votes, num_clients, const client_indices[],
	const client_votes[], num_items, const item_indices[], const item_votes[])
{
	new Handle:results = ConvertVoteResults(menu, num_clients, client_indices, client_votes, num_items, item_indices);
...
Handle:ConvertVoteResults(Handle:menu, num_clients, const client_indices[], const client_votes[], num_items, const item_indices[])
{
	...
	for (new i = 0; i < num_items; i++)
	{
		...
		for (new j = 0; j < num_clients; j++)
		{
			...
			{
				PushArrayCell(voteClientArray, client_indices[j]);
			}
		}
	}
