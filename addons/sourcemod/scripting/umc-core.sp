...
public Native_UMCVoteManagerComplete(Handle:plugin, numParams)
{
	...
	new Function:handler = GetNativeFunction(3);
...
bool:PerformVote(Handle:voteManager, UMC_VoteType:type, Handle:options,
	time, const clients[], numClients, const String:startSound[])
{
	new Handle:handler;
	switch (type)
	{
										...
										bool:exclude=true)
{
	...
	new nomIndex, position, numMapsFromCat, nomCounter, inVote, index;
	...
	do
	{
			...
			if (numNoms > 0)
			{
				...
				for (new i = 0; i < numNoms; i++)
				{
					...
					if (!ignoreDupes && FindStringInVoteArray(mapName, MAP_TRIE_MAP_KEY, map_vote) != -1)
					{
						if (verboseLogs)
						{
							LogUMCMessage("VOTE MENU: (Verbose) Skipping nominated map '%s' from map group '%s'"
								... " because it already is in the vote.", mapName, catName);
						}
					}
					else
					{
		...
		else
		{
			...
			for (new i = 0; i < numNoms; i++)
			{
				...
				if (!ignoreDupes && FindStringInVoteArray(mapName, MAP_TRIE_MAP_KEY, map_vote) != -1)
				{
					...
					{
						LogUMCMessage("VOTE MENU: (Verbose) Skipping nominated map '%s' from map group '%s'"
							... " because it is already in the vote.", mapName, catName);
					}
				}
				else
				{
		...
		while (numMapsFromCat > 0)
		{
			...
			if (!ignoreDupes && FindStringInVoteArray(mapName, MAP_TRIE_MAP_KEY, map_vote) != -1)
			{
				...
				if (verboseLogs)
				{
					LogUMCMessage("VOTE MENU: (Verbose) Skipping selected map '%s' from map group '%s'"
						... " because it is already in the vote.", mapName, catName);
				}
	...
	if (NeedRunoff(vM))
	{
		...
		if (remaining_runoffs == 0 || prev_vote_count == 2)
		{
			...
			else if (stored_fail_action == RunoffFailAction_Nothing)
			{
				...
				PrintToChatAll("[UMC] %t (%t)", "Vote Failed", "Vote Win Percentage", percentage, total_votes);
...
DoRunoffVote(Handle:vM, Handle:response)
{
	...
	if (runoffOptions != INVALID_HANDLE)
	{
		...
		new Handle:pack;
		CreateDataTimer(1.0, Handle_RunoffVoteTimer, pack, TIMER_REPEAT | TIMER_FLAG_NO_MAPCHANGE);
...
Handle:BuildRunoffOptions(Handle:vM, Handle:clientArray)
{
	...
	for (new i = num_items - 1; i > 1; i--)
	{
		...
		if (verboseLogs)
		{
			...
			LogUMCMessage("RUNOFF MENU: (Verbose) '%s' was removed from the vote. It had %i votes (%.f%% of total)",
	...
	if (verboseLogs)
	{
		LogUMCMessage("RUNOFF MENU: (Verbose) Stopped removing options from the vote."
			... " Maximum possible winning vote percentage is %.f%%.", percent);
	}
...
public Handle_MapVoteWinner(Handle:vM, const String:info[], const String:disp[], Float:percentage)
{
	...
	{
		PrintToChatAll("[UMC] %t %t (%t)", "End of Map Vote Over", "Map Extended",
			"Vote Win Percentage", percentage, total_votes);
		LogUMCMessage("MAPVOTE: Players voted to extend the map.");
		ExtendMap(vM);
	}
	else if (StrEqual(info, DONT_CHANGE_OPTION))
	{
		PrintToChatAll("[UMC] %t %t (%t)", "End of Map Vote Over", "Map Unchanged",
			"Vote Win Percentage", percentage, total_votes);
	...
	{
		PrintToChatAll("[UMC] %t %t (%t)", "End of Map Vote Over", "End of Map Vote Map Won",
			disp, "Vote Win Percentage", percentage, total_votes);
							...
							Float:percentage)
{
	...
	else //Otherwise, we pick a random map from the category and set that as the next map.
	{
		...
		PrintToChatAll("[UMC] %t %t (%t)", "End of Map Vote Over", "End of Map Vote Group Won",
			map, cat, "Vote Win Percentage", percentage, total_votes);
...
bool:GetRandomMapFromCycle(Handle:kv, const String:group[], String:buffer[], size, String:gBuffer[], gSize)
{
